...
class nexus_zk(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if config is None:
            logger.error("Config node for nexus_zk missing.")
            raise SystemExit(1)
        self.num_jobs = str(config['jobs'] if 'jobs' in config else 1)
        self.pluginpath = os.path.abspath(config['pluginpath'])
        self.nexus_exe = os.path.join(config['PATH'] if 'PATH' in config else "", "nexus")
        self.isa_spec = os.path.abspath(config['ispec']) if 'ispec' in config else ''
        self.platform_spec = os.path.abspath(config['pspec']) if 'ispec' in config else ''
        self.make = config['make'] if 'make' in config else 'make'
        logger.debug("Nexus zkVM plugin initialized using the following configuration.")
        for entry in config:
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        if shutil.which(compiler) is None:
            ...
            raise SystemExit(1)
        if shutil.which(self.nexus_exe) is None:
            logger.error(self.nexus_exe+ ": executable not found. Please check environment setup.")
            raise SystemExit(1)
    ...
    def runTests(self, testList, cgf_file=None, header_file=None):
        ...
        for file in testList:
            ...
            execute += f"{self.nexus_exe} execute {elf} --signature={sig_file} --signature-granularity=4 > {test_name}.log 2>&1;"
